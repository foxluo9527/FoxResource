...
interface MusicDAO {
    ...
    // 批量插入（自动处理艺人）
    @Transaction
    suspend fun insertMusics(musics: List<MusicEntity>): List<Long> {
        // 提取并去重艺人
    ...
    suspend fun _insertMusic(music: MusicEntity): Long

    @Insert(onConflict = OnConflictStrategy.IGNORE)
    suspend fun _insertMusics(musics: List<MusicEntity>): List<Long>
    ...
    suspend fun getMusicsByIds(musicIds: List<String>): List<MusicEntity>
